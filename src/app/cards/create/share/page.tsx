            ...
            <div className="rounded-xl p-4 mb-4" style={{ background: "var(--color-white)", border: "1px solid var(--color-light-gray)" }}>
              ...
              <div className="flex justify-center">
                <div className="bg-white rounded-lg shadow-md overflow-hidden relative" style={{ width: 300, height: cardSize === "4x6" ? 400 : 420, border: "1px solid var(--color-light-gray)" }}>
                  ...
                  {insideImagePosition === "corner_flourish" && (<div className="relative h-full">{accentPositions.map((slot) => (<div key={slot} style={cornerStyle(slot)}><img src={insideImageUrl} alt="" style={cornerImgStyle()} /></div>))}<div className="relative flex flex-col items-center justify-center h-full px-4" style={{ padding: accentPositions.length > 2 ? "2.5rem 1.5rem" : "1rem" }}><div className="w-3/4 space-y-1.5">{[1,2,3,4].map(l => <div key={l} className="h-1.5 rounded bg-light-gray" style={{ width: l === 4 ? "50%" : "100%" }} />)}</div></div></div>)}
                  {insideImagePosition === "frame" && (<div className="relative h-full"><img src={insideImageUrl} alt="" style={frameImgStyle()} /><div className="relative flex flex-col items-center justify-center h-full" style={{ padding: "15% 12%" }}><div className="w-3/4 space-y-1.5">{[1,2,3,4].map(l => <div key={l} className="h-1.5 rounded bg-light-gray" style={{ width: l === 4 ? "50%" : "100%" }} />)}</div></div></div>)}
                  {insideImagePosition === "top_edge_accent" && (<div className="flex flex-col h-full">{accentPositions.includes(1) && (<div style={edgeStyle(1)}><img src={insideImageUrl} alt="" style={edgeImgStyle()} /></div>)}<div className="flex-1 flex flex-col items-center justify-center px-4"><div className="w-3/4 space-y-1.5">{[1,2,3,4].map(l => <div key={l} className="h-1.5 rounded bg-light-gray" style={{ width: l === 4 ? "50%" : "100%" }} />)}</div></div>{accentPositions.includes(2) && (<div style={edgeStyle(2)}><img src={insideImageUrl} alt="" style={edgeImgStyle()} /></div>)}</div>)}
                </div>
              </div>
            </div>
            <div className="rounded-xl p-4 mb-6" style={{ background: "var(--color-white)", border: "1px solid var(--color-light-gray)" }}>
